...
#         if form.is_valid():
#             form.save()
#             return redirect('doctor-dashboard')  # Redirect to a profile page or any other page
#     else:
#         form = DoctorForm(instance=doctor)
#     return render(request, 'hospital/doctor_update.html', {'form': form, 'doctor':doctor})



# @login_required(login_url='doctorlogin')
# @user_passes_test(is_doctor)
# def create_treatment_history(request, id):
#     TreatmentForm= forms.TreatmentHistoryForm()
#     patient = models.Patient.objects.all().filter(status=True, id=id, assignedDoctorId=request.user.id)
#     if request.method == 'POST':
#         TreatmentForm = forms.TreatmentHistoryForm(request.POST)
...
#         if form.is_valid():
#             form.save()
#             return redirect('patient-dashboard')  # Redirect to a profile page or any other page
#     else:
#         form = PatientForm(instance=patient)
#     return render(request, 'hospital/patient_update.html', {'form': form})
...
from .models import Patient, PatientDischargeDetails
...
def create_checkout_session(request):
    ...
    # Fetch the current patient's discharge details
    patient = get_object_or_404(Patient, user_id=request.user.id)
    discharge_details = PatientDischargeDetails.objects.filter(patientId=patient.id).order_by('-id').first()

    if not discharge_details:
        return render(request, 'hospital/error.html', {'message': 'No discharge details found.'})

    if request.method == 'POST':
        # Create a line item based on the latest discharge details
        line_items = [
            {
                'price_data': {
                    'currency': 'INR',
                    'unit_amount': int(discharge_details.total * 100),  # Convert to cents
                    'product_data': {
                        'name': f"Discharge Details for {discharge_details.patientName}",
                    },
                },
                'quantity': 1,
            },
    ...
    # Optionally render a form for initiating checkout if it's not a POST request
    return render(request, 'hospital/checkout_form.html', {'discharge_details': discharge_details})

def success(request):
    # Implement any logic needed after a successful payment
    return render(request, 'hospital/success.html')

def cancel(request):
    # Implement any logic needed if the payment is cancelled
    return render(request, 'hospital/cancel.html')
